! ...
! Odredjivanje Bx komponente magnetskog polja, preko vektorskog magnetskog potencijala.
...
subroutine Hpolje_Bx(N,Ns,isl,xt,yt,zt,xp,yp,zp,xk,yk,zk,iso,r0,L,Iu,HD,gama,Bx)
    ...
    real(8) u,v,DuDy,DvDy,DFDu,DFDv,DFDy,DuDz,DvDz,DFDz,temp1,temp2
    real(8),parameter :: pi = 3.141592653589793238d0
    real(8) mio,konst,Rs
    complex(8) gamae,Fprig,ggama
    complex(8) DAzDy,DAyDz
    ...
    DO iseg = 1,Ns
        ...
        ! Derivacija Az po y
        DAzDy = konst * (((z2-z1)/L(iseg))*Iu(iseg) * (-ggama*Fprig*((yt-ysi)/Rs)*andi + Fprig*DFDy))
        ...
        ! Derivacija Ay po z
        DAyDz = konst * (((y2-y1)/L(iseg))*Iu(iseg) * (-ggama*Fprig*((zt-zsi)/Rs)*andi + Fprig*DFDz))
