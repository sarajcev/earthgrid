! ...
! Proracun Fiy komponente potrebne za proracun Ey komponente elektricnog polja.
! Naime, vrijedi opcenito sljedeci izraz:

!             @Fi
!     Ey = - ----- - j*omega*Ay = -Fiy -j*omega*Ay
!              @y

! Ova subroutina racuna Fiy za potrebe gornjeg izraza, i to od ukupnog broja
! segmenata, dakle, ukupni Fiy (od svih segmenata). U ovoj subroutini impleme-
...
subroutine Epolje_Fiy(N,Ns,Nsi,isl,xt,yt,zt,xp,yp,zp,xk,yk,zk,Fr,iso,r0,&
	...
	real(8) u,v,DuDy,DvDy,DFDu,DFDv,DFDy,temp1,temp2
	integer ngt1
	...
	! --------------------------------------------------------------------------------
	!                POCETAK PRORACUNA Fiy ZA PRORACUN Ey
	! --------------------------------------------------------------------------------
	Nss = Ns - Nsi
	...
	IF (N==2) THEN
		...
		DO iseg = 1,Nss
			...
			if (dabs(z2-z1)<=0.001) then
				...
				prvi_dio = faktorp*(-ggama)*Fprig*((yt-ysi)/Rs)*integral
				! ----------------------------------------------------
				integral = dcmplx(0.d0,0.d0)
				do j = 1,3
					zsi = zm(j); z2 = zm(j)
					...
					if (v < pol) v = pol
					DuDy = (y2-y1)/L(iseg)
					DvDy = (1.d0/v) * (yt-ysi-u*((y2-y1)/L(iseg)))
	...
	ELSE
		...
		DO iseg = 1,Nss
			...
			if (dabs(z2-z1)<0.001) then
				...
				prvi_dio = faktorp*(Ip(iseg)/L(iseg))*(-ggama)*Fprig*((yt-ysi)/Rs)*integral
				! ------------------------------------------------------
				integral = dcmplx(0.d0,0.d0)
				do j = 1,33
					zsi = zm(j); z2 = zm(j)
		...
		Fiy = Fiy + (prvi_dio + drugi_dio + treci_dio + cetvrti_dio)
		END DO
	END IF
