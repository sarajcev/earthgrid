! ...
! Proracun Fix komponente potrebne za proracun Ex komponente elektricnog polja.
! Naime, vrijedi opcenito sljedeci izraz:

!             @Fi
!     Ex = - ----- - j*omega*Ax = -Fix -j*omega*Ax
!              @x

! Ova subroutina racuna Fix za potrebe gornjeg izraza, i to od ukupnog broja
! segmenata, dakle, ukupni Fix (od svih segmenata). U ovoj subroutini impleme-
...
subroutine Epolje_Fix(N,Ns,Nsi,isl,xt,yt,zt,xp,yp,zp,xk,yk,zk,Fr,iso,r0,&
	...
	real(8) u,v,DuDx,DvDx,DFDu,DFDv,DFDx,temp1,temp2
	integer ngt1
	...
	! --------------------------------------------------------------------------------
	!                POCETAK PRORACUNA Fix ZA PRORACUN Ex
	! --------------------------------------------------------------------------------
	Nss = Ns - Nsi
	...
	IF (N==2) THEN
		...
		DO iseg = 1,Nss
			...
			if (dabs(z2-z1)<=0.001) then
				...
				prvi_dio = faktorp*(-ggama)*Fprig*((xt-xsi)/Rs)*integral
				! ----------------------------------------------------
				integral = dcmplx(0.d0,0.d0)
				do j = 1,3
					zsi = zm(j); z2 = zm(j)
	...
	ELSE
		...
		DO iseg = 1,Nss
			...
			if (dabs(z2-z1)<0.001) then
				...
				prvi_dio = faktorp*(Ip(iseg)/L(iseg))*(-ggama)*Fprig*((xt-xsi)/Rs)*integral
				! ------------------------------------------------------
				integral = dcmplx(0.d0,0.d0)
				do j = 1,33
					zsi = zm(j); z2 = zm(j)
		...
		Fix = Fix + (prvi_dio + drugi_dio + treci_dio + cetvrti_dio)
		END DO
	END IF
