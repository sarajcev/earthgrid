! ...
! Odredjivanje Bz komponente magnetskog polja, preko vektorskog magnetskog potencijala.
...
subroutine Hpolje_Bz(N,Ns,isl,xt,yt,zt,xp,yp,zp,xk,yk,zk,iso,r0,L,Iu,HD,gama,Bz)
    ...
    real(8) u,v,DuDx,DvDx,DFDu,DFDv,DFDx,DuDy,DvDy,DFDy,temp1,temp2
    real(8),parameter :: pi = 3.141592653589793238d0
    real(8) mio,konst,Rs
    complex(8) gamae,Fprig,ggama
    complex(8) DAyDx,DAxDy
    ...
    DO iseg = 1,Ns
        ...
        ! Derivacija Ay po x
        DAyDx = konst * (((y2-y1)/L(iseg))*Iu(iseg) * (-ggama*Fprig*((xt-xsi)/Rs)*andi + Fprig*DFDx))
        ...
        ! Derivacija Ax po y
        DAxDy = konst * (((x2-x1)/L(iseg))*Iu(iseg) * (-ggama*Fprig*((yt-ysi)/Rs)*andi + Fprig*DFDy))
